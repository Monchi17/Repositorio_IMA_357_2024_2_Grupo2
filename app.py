...
df = pd.read_csv(url, usecols=['Titular', 'Cuerpo']) #tomo solo las columnas Titular y cuerpo

st.title("App Grupo 2 :)")
...
if oracion:
    ...
    st.write(pd.DataFrame({'Titular': [doc_max_similitud['Titular']], 'Similitud': [similitudes[indice_max_similitud]]})) #Resultado

    st.write("Documento con mayor suma de frecuencias de tokens de la oración:")
    st.write(pd.DataFrame({'Titular': [doc_max_frecuencia_tokens['Titular']], 'Suma de Frecuencias': [doc_max_frecuencia_tokens['Suma_frecuencias']]})) #Resultado
